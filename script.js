...
function drawClock() {
    ...
    img.onload = () => {
        ...
        clockInterval = setInterval(() => {
            ...
            let angleSeconds = degreesSeconds * 2 * Math.PI / 360;
            let x1 = x + radiusSeconds * Math.cos(angleSeconds);
            let y1 = y + radiusSeconds * Math.sin(angleSeconds);
            drawLine(x, y, x1, y1, 'green', widthLine);

            let angleMinutes = degreesMinutes * 2 * Math.PI / 360;
...
// prisma rectangular esto se modifico 

const prismWidth = 140;
const prismHeight = 80;
const prismDepth = 60;

const halfW = prismWidth / 2;
const halfH = prismHeight / 2;
const halfD = prismDepth / 2;

let prismVertices = [
    { x: -halfW, y: -halfH, z: -halfD }, // 0
    { x:  halfW, y: -halfH, z: -halfD }, // 1
    { x:  halfW, y:  halfH, z: -halfD }, // 2
    { x: -halfW, y:  halfH, z: -halfD }, // 3
    { x: -halfW, y: -halfH, z:  halfD }, // 4
    { x:  halfW, y: -halfH, z:  halfD }, // 5
    { x:  halfW, y:  halfH, z:  halfD }, // 6
    { x: -halfW, y:  halfH, z:  halfD }, // 7
];


const prismFaces = [
    { indices: [0, 1, 2, 3], color: 'red'    }, // frente
    { indices: [5, 4, 7, 6], color: 'red'    }, // atrás
    { indices: [4, 0, 3, 7], color: 'blue'   }, // izquierda
    { indices: [1, 5, 6, 2], color: 'blue'   }, // derecha
    { indices: [4, 5, 1, 0], color: 'green'  }, // abajo
    { indices: [3, 2, 6, 7], color: 'green'  }, // arriba
];

//hasta aqui y tambien lo del prism
